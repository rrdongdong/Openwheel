...
class ThesisGUI:
    ...
    def _build_ui(self):
        ...
        tk.Label(self.root, text="润色位置").grid(row=3, column=0, sticky="w", **pad)
        tk.Entry(self.root, textvariable=self.position_var, width=70).grid(row=3, column=1, columnspan=3, sticky="we", **pad)

        tk.Label(self.root, text="待润色文字").grid(row=4, column=0, sticky="nw", **pad)
        self.input_text = tk.Text(self.root, height=10, width=90)
        self.input_text.grid(row=4, column=1, columnspan=3, sticky="we", **pad)

        tk.Button(self.root, text="开始润色", command=self.run_polish).grid(row=5, column=0, columnspan=4, sticky="we", **pad)

        tk.Label(self.root, text="润色后输出").grid(row=6, column=0, sticky="nw", **pad)
        self.output_text = tk.Text(self.root, height=10, width=90)
        self.output_text.grid(row=6, column=1, columnspan=3, sticky="we", **pad)

        tk.Label(self.root, text="状态").grid(row=7, column=0, sticky="nw", **pad)
        self.status_text = tk.Text(self.root, height=6, width=90)
        self.status_text.grid(row=7, column=1, columnspan=3, sticky="we", **pad)
    ...
    def run_outline(self):
        ...
        threading.Thread(target=self._run_subprocess, args=(cmd, lambda: self.append_status("大纲提取完成")), daemon=True).start()
